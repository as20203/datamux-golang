...
fi,err:=strconv.Atoi(first+first1+first2)
//fi1,err:=strconv.Atoi(first1)
//fi2,err:=strconv.Atoi(first2)
fi3,err:=strconv.ParseInt(first3 ,16,64)
...
var strAbs,strRev string
	var parsedvalues []SmartValveData
              ...
              //fmt.Println("len(databytes):", len(databytes))
				...
				sat, err :=strconv.ParseInt(hexaNumberToInteger(strAbs), 16, 64)//strconv.Atoi(strAbs)
				sat1,err :=strconv.ParseInt(hexaNumberToInteger(strRev), 16, 64)//strconv.Atoi(strRev)
				//fmt.Println("sat=",sat)	
				var j,k float64
