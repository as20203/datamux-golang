...
func parseLandisGyrData(receivedtime time.Time, port uint8, receiveddata string) []LandisGyrData {
	...
	strMR, _ := strconv.ParseInt(strMRHex, 10, 64)
	strVol, _ := strconv.ParseInt(strVolHax, 10, 64)
	strPower, _ := strconv.ParseInt(strPowerHax, 10, 64)
	strFlow, _ := strconv.ParseInt(strFlowHax, 10, 64)
	strFwdTemp, _ := strconv.ParseInt(strFwdTempHex, 10, 64)
