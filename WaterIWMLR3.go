...
var strAbs,strRev string
	var parsedvalues []WaterIWMLR3Data
              ...
              fmt.Println("len(databytes):", len(databytes))
				...
				sat, err :=strconv.ParseInt(hexaNumberToInteger(strAbs), 10, 64)//strconv.Atoi(strAbs)
				sat1,err :=strconv.ParseInt(hexaNumberToInteger(strRev), 10, 64)//strconv.Atoi(strRev)
